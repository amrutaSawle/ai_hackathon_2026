...
def calculateRisk(payment):
    ...
    if iban_valid == 0 and payment.new_beneficiary:
        score = max(score, 90)
        reasons.insert(
            0,
            "Critical fraud rule: Invalid IBAN with new beneficiary."
        )
    elif (
            iban_valid == 0
            and payment.transaction_amount >= 10000
        ):
            score = max(score, 85)

            reasons.insert(
                0,
                "Critical fraud rule: Invalid IBAN with high-value transaction."
            )
    ...
    # AI prediction OR very high risk score => Fraud
    if ai_prediction == 1 or score >= 85:
        prediction = 1
    else:
        prediction = 0
    ...
    summary = (
        "Transaction is highly suspicious."
    )
    ...
    actions = [
        ...
        recommendation = (
            "Verify beneficiary before proceeding."
        )
